...
import { normalizeGame, computeStandings } from './_standingsLogic.js'
...
export async function getStandings() {
  ...
  return computeStandings({
    players: playerRows.map(toObj),
    sessions: weekRows.map(toObj),
    games: gameRows.map(toObj),
    teamPlayers: tpRows.map(toObj),
    departures: depRows.map(toObj),
    exclusions: exclRows.map(toObj),
    historicalStats: histRows.map(toObj),
  })
}
